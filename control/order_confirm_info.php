...
/*
        $query = "SELECT count(*) FROM wx_order_send ";
...
*/
        ...
        <table class="table table-bordered table-hover">
            ...
            <?php
            //                    $url="SELECT * FROM WX_Qrscene_Info WHERE parent_id='' and qrscene_name like '%".$keyword."%' order by id asc limit ".$offset.",".$pagesize;


            $result = mysql_query("SELECT * FROM wx_order_confirm  order by id DESC limit " . $offset . "," . $pagesize . "", $link);
            while ($row = mysql_fetch_array($result)) {

                $query = "SELECT * FROM wx_order_send WHERE SellID='".$row["SellID"]."'";

//                echo $query;

                $rs = mysql_query($query);
                $rows = mysql_fetch_array($rs);
                $confirmdate = $rows["AddDate"];
                ...
                echo "<td >" . $row["WX_OpenID"] . "</td>";
                echo "<td >" . $row["AddDate"] . "</td>";
                echo "<td >".$confirmdate."</td>";
                echo "<td >" . $row["focusdate"] . "</td>";
                echo "<td ><a href='http://weix2.hengdianworld.com/server/wechat/inc/send_kefu.php?sellid=".$row["SellID"]."&openid=".$row["WX_OpenID"]."' target='_blank'> 咨询</a></td>";
                echo "<tr>";
        ...
        if ($page > 1) {
            echo "<li><a href='order_confirm_info.php?page=" . $first . "'>首页</a></li>";
            echo "<li><a href='order_confirm_info.php?page=" . $prev . "'>上一页</a></li>";
        }

        //                echo " 共有" .$pages. "页(" .$page. "/" .$pages.")";
        for ($i = 1; $i < $page; $i++)
            echo "<li><a href='order_confirm_info.php?page=" . $i . "'>" . $i . "</a></li>";
        echo "<li class='active'><a href='#'>" . $page . "</a></li>";
        for ($i = $page + 1; $i <= $pages; $i++)
            echo "<li><a href='order_confirm_info.php?page=" . $i . "'>" . $i . "</a></li>";
        if ($page < $pages) {
            echo "<li><a href='order_confirm_info.php?page=" . $next . "'>下一页</a></li>";
            echo "<li><a href='order_confirm_info.php?page=" . $last . "'>最后一页</a></li>";
        }
        echo "</ul>";
        echo "</div>";
        ?>
