...
function query_dm($fromUsername,$toUsername,$ParentID)
{
	...
	{
		responseV_Text($fromUsername,"不好意思，您未参加过猜灯谜活动或猜谜失败。如果您要参加梦幻谷猜灯谜活动，请点击菜单横店攻略中的最新活动。");
	}
	else
	{
	  while($row=mysql_fetch_array($result))
...
*
*/
function Query_ggl($fromUsername)
{
    include ("mysql.php");
//	include ('../../inc/response.php');
//	include ('../../inc/function.php');
    ...
    if(mysql_num_rows($result)==0)
    {
        responseV_Text($fromUsername,"不好意思，您未刮到过奖品，再接再厉哦。如果您要参加刮刮乐活动，请点击菜单活动专区中的最新活动。");
    }
    else
    {
        while($row = mysql_fetch_array($result))
...
*
*/
function Query_yyy($fromUsername)
{
	include ("mysql.php");
//	include ('../../inc/response.php');
//	include ('../../inc/function.php');
	...
	if(mysql_num_rows($result)==0)
	{
		responseV_Text($fromUsername,"不好意思，您未摇到过奖品，再接再厉哦。");
	}
	else
	{
		while($row = mysql_fetch_array($result))
